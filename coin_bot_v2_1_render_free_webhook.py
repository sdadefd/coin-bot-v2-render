...
def main():
    ...
    # Render usually provides one of these public URL variables after Web Service creation.
    # If none exists, set WEBHOOK_URL manually in Render Environment Variables:
    ...
    if not webhook_url:
        raise RuntimeError(
            "WEBHOOK_URL missing. In Render Environment Variables add: "
            "WEBHOOK_URL=https://YOUR-SERVICE-NAME.onrender.com/telegram-webhook"
        )

    print(
        f"Coin Bot v2.1 Render Free Webhook Started... "
        f"ADMIN_IDS={ADMIN_IDS}, REF_BONUS={get_ref_bonus()}, ORDER_COST={get_order_cost()}, "
        f"PORT={port}, WEBHOOK_PATH=/{webhook_path}"
    )
...
if __name__ == "__main__":
    # Python 3.14/3.15 compatibility fix.
    try:
